...
def fix_ticket_timestamps():
    ...
    try:
        ...
        for ticket in tickets_to_process:
            ...
            # Convert creado_en
            if original_creado_en and original_creado_en.tzinfo is None: # Only process naive datetimes
            ...
            if original_actualizado_en and original_actualizado_en.tzinfo is None: # Only process naive datetimes
                # Assume the stored datetime is naive Argentina time
                localized_dt_arg = ARGENTINA_TIMEZONE.localize(original_actualizado_en)
                utc_dt = localized_dt_arg.astimezone(UTC_TIMEZONE)
                ticket.actualizado_en = utc_dt
            
            if (original_creado_en and original_creado_en.tzinfo is None and original_creado_en != ticket.creado_en) or \
               (original_actualizado_en and original_actualizado_en.tzinfo is None and original_actualizado_en != ticket.actualizado_en):
                updated_count += 1
                print(f"Ticket ID {ticket.id}: Creado_en changed from {original_creado_en} to {ticket.creado_en}, Actualizado_en changed from {original_actualizado_en} to {ticket.actualizado_en}")
