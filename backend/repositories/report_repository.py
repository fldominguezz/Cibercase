...
class ReportRepository:
    def get_ticket_counts_by_status(self, db: Session):
        return (
            db.query(Ticket.estado, func.count(Ticket.id)).group_by(Ticket.estado).all()
        )

    def get_ticket_counts_by_severity(self, db: Session):
        return (
            db.query(Ticket.severidad, func.count(Ticket.id))
            .group_by(Ticket.severidad)
            .all()
        )
        ...
        for i in range(8):  # Last 8 weeks
            ...
            count = (
                ...
                .filter(
                    ...
                    func.date(Ticket.creado_en)
                    <= end_of_week + timedelta(days=6),  # End of the week (Sunday)
                )
                .count()
            )
    ...
    def get_my_assigned_tickets_count(self, db: Session, user_id: int) -> int:
        return (
            db.query(Ticket)
            .filter(Ticket.asignado_a_id == user_id, Ticket.estado != "Cerrado")
            .count()
        )

    def get_ticket_counts_by_category(self, db: Session):
        counts = (
            db.query(Ticket.categoria, func.count(Ticket.id))
            .group_by(Ticket.categoria)
            .all()
        )
        logger.info(
            f"DEBUG: Raw counts from get_ticket_counts_by_category in repository: {counts}"
        )
        return counts
    ...
    def get_avg_response_time(self, db: Session) -> float:
        ...
        avg_time = (
            db.query(
                func.avg(extract("epoch", Ticket.actualizado_en - Ticket.creado_en))
            )
            .filter(Ticket.asignado_a_id.isnot(None), Ticket.actualizado_en.isnot(None))
            .scalar()
        )
    ...
    def get_avg_resolution_time(self, db: Session) -> float:
        ...
        avg_time = (
            db.query(func.avg(extract("epoch", Ticket.cerrado_en - Ticket.creado_en)))
            .filter(Ticket.cerrado_en.isnot(None))
            .scalar()
        )
        return avg_time / 3600 if avg_time else 0.0  # Convert seconds to hours
    ...
    def get_top_recurring(self, db: Session, limit: int = 5) -> List[Dict[str, Any]]:
        ...
        recurring_alerts = (
            db.query(Ticket.rule_name, func.count(Alert.id).label("count"))
            .join(Ticket, Alert.ticket_id == Ticket.id)
            .filter(Ticket.rule_name.isnot(None))
            .group_by(Ticket.rule_name)
            .order_by(func.count(Alert.id).desc())
            .limit(limit)
            .all()
        )
        return [
            {"name": item.rule_name, "count": item.count} for item in recurring_alerts
        ]
