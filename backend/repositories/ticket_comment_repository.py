...
from db.models import TicketComment  # Import User to fetch user details
...
class TicketCommentRepository(
    BaseRepository[TicketComment, schemas.ticket_comment.TicketCommentCreate, None]
):
    def __init__(self):
    ...
    def create_comment(
        self,
        db: Session,
        obj_in: schemas.ticket_comment.TicketCommentCreate,
        user_id: int,
        ticket_id: int,
    ) -> schemas.ticket_comment.TicketComment:
        db_obj = TicketComment(
            content=obj_in.content, user_id=user_id, ticket_id=ticket_id
        )
        db.add(db_obj)
        db.commit()
        db.refresh(db_obj)
        ...
        try:
            audit_log_data = AuditLogBase(
                ...
                detalle=json.dumps(
                    {"ticket_id": ticket_id, "content_preview": obj_in.content[:50]}
                ),  # Log first 50 chars of content
            )
