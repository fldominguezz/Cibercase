...
class AuditLogRepository(BaseRepository[AuditLog, AuditLogBase, AuditLogBase]):
    def get_by_ticket_id(self, db: Session, *, ticket_id: int) -> List[AuditLog]:
        return db.query(AuditLog).filter(AuditLog.entidad == "Ticket", AuditLog.entidad_id == ticket_id).order_by(AuditLog.timestamp.desc()).all()

    def get_multi_with_actor_details(self, db: Session, *, skip: int = 0, limit: int = 100) -> List[Tuple[AuditLog, str]]:
        """
