...
try:
    ...
    cur = conn.cursor()
    cur.execute("SELECT id, resumen, categoria, platform, severidad, estado FROM tickets WHERE id = 3")
    ticket = cur.fetchone()
    if ticket:
        ...
        if ticket[2] == 'Malware' and ticket[3] == 'FortiSIEM':
            ...
            print(f"Verification failed: Category is '{ticket[2]}' (expected 'Malware'), Platform is '{ticket[3]}' (expected 'FortiSIEM').")
    else:
