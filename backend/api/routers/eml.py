...
def check_suspicious_attachment(filename, content_type):
    suspicious_extensions = ['.exe', '.zip', '.js', '.vbs', '.scr', '.bat', '.cmd', '.ps1', '.dll', '.chm', '.hta']
...
def check_phishing_indicators(subject, body, headers):
    ...
    urgency_keywords = ['urgente', 'acción requerida', 'su cuenta será suspendida', 'verifique su cuenta', 'caduca']
    ...
    credential_keywords = ['contraseña', 'usuario', 'credenciales', 'iniciar sesión', 'verificar ahora']
    ...
    if from_header and return_path and from_header != return_path and "via" not in from_header:
        indicators.append(f"Posible suplantación de identidad: 'From' ({from_header}) difiere de 'Return-Path' ({return_path}).")
...
async def scan_eml_file(eml_file: UploadFile = File(...)):
        ...
        for part in msg.walk():
            ...
            if part.get_filename(): # Es un adjunto
                result["attachments"].append({
                    ...
                    "is_suspicious": check_suspicious_attachment(part.get_filename(), content_type)
                })
    ...
    for url in extracted_urls:
        ...
        if "login" in url.lower() and "example.com" not in url.lower(): # Placeholder for actual domain
            malicious_links.append(f"URL sospechosa (posible phishing): {url}")
        elif "update" in url.lower() and "legit-domain.com" not in url.lower(): # Placeholder
             malicious_links.append(f"URL sospechosa (posible actualización falsa): {url}")

    phishing_indicators = check_phishing_indicators(result["subject"], result["body"], result["headers"])

    result["security_analysis"] = {
        "malicious_links": malicious_links if malicious_links else "No se detectaron enlaces maliciosos obvios.",
        "suspicious_attachments": [att for att in result["attachments"] if att["is_suspicious"]],
        "phishing_indicators": phishing_indicators if phishing_indicators else "No se detectaron indicadores de phishing obvios.",
    }
