...
async def create_notification(
    db: Session, user_id: int, message: str, link: Optional[str] = None
):
...
@router.post(
    ...
    description="Creates a new support ticket with the provided details and optional file attachments. The current authenticated user will be set as the reporter.",
)
async def create_ticket(
    ...
    estado: str = Form(
        ...,
        description="Current status of the ticket (e.g., 'Nuevo', 'En Progreso', 'Resuelto', 'Cerrado').",
    ),
    severidad: str = Form(
        ...,
        description="Severity level of the ticket (e.g., 'Baja', 'Media', 'Alta', 'Crítica').",
    ),
    resumen: str = Form(..., description="A brief summary or title of the ticket."),
    descripcion: Optional[str] = Form(
        None, description="Detailed description of the issue or request."
    ),
    impacto: Optional[str] = Form(None, description="Impact level of the ticket."),
    causa_raiz: Optional[str] = Form(
        None, description="Identified root cause of the issue."
    ),
    asignado_a_id: Optional[int] = Form(
        None, description="ID of the user assigned to the ticket."
    ),
    sla_vencimiento: Optional[datetime] = Form(
        None, description="Service Level Agreement (SLA) due date."
    ),
    categoria: Optional[str] = Form(
        None, description="Category of the ticket (e.g., 'Incidente', 'Solicitud')."
    ),
    files: List[UploadFile] = File(
        [], description="Optional files to attach as evidence to the ticket."
    ),
) -> Any:
    """
    Create new ticket.
    ...
    ticket = await ticket_service.create_ticket(
        db=db, ticket_data=ticket_in, files=files, current_user_id=current_user.id
    )
...
@router.get(
    ...
    description="Retrieves a paginated list of tickets, with extensive filtering, sorting, and search capabilities. Users can filter by status, severity, category, assigned user, reporter, and search across multiple ticket fields.",
)
def read_tickets(
    db: Session = Depends(deps.get_db),
    skip: int = 0,
    ...
    return JSONResponse(
        content=jsonable_encoder({"total_count": total_count, "tickets": tickets})
    )
...
@router.get(
    ...
    description="Retrieves the list of available ticket categories.",
)
...
@router.get(
    ...
    description="Retrieves the detailed information for a specific ticket, including its comments and evidence, identified by its unique ID.",
)
def read_ticket(
    ticket_id: int,
    db: Session = Depends(deps.get_db),
...
@router.put(
    ...
    description="Updates the details of an existing ticket. Users with 'Admin' or 'Lider' roles have full update permissions, while other roles may have restricted update capabilities (e.g., cannot change status or assignee).",
)
async def update_ticket(
    ...
    severidad: Optional[str] = Form(
        None,
        description="Severity level of the ticket (e.g., 'Baja', 'Media', 'Alta', 'Crítica').",
    ),
    resumen: Optional[str] = Form(
        None, description="A brief summary or title of the ticket."
    ),
    descripcion: Optional[str] = Form(
        None, description="Detailed description of the issue or request."
    ),
    ...
    resolucion: Optional[str] = Form(
        None, description="Resolution details for the ticket."
    ),
    ...
    if (
        updated_ticket.asignado_a_id
        and updated_ticket.asignado_a_id != old_assigned_to_id
    ):
        assigned_user = (
            db.query(User).filter(User.id == updated_ticket.asignado_a_id).first()
        )
        if assigned_user:
    ...
    if updated_ticket.estado != old_estado:
        ...
        if updated_ticket.reportado_por_id:
            reporter_user = (
                db.query(User)
                .filter(User.id == updated_ticket.reportado_por_id)
                .first()
            )
            if reporter_user:
        ...
        if (
            updated_ticket.asignado_a_id
            and updated_ticket.asignado_a_id != updated_ticket.reportado_por_id
        ):
            assigned_user = (
                db.query(User).filter(User.id == updated_ticket.asignado_a_id).first()
            )
...
@router.post(
    ...
    description="Adds a new comment to a specified ticket. The comment is associated with the current authenticated user.",
)
async def create_ticket_comment(
    ...
    try:
        ...
        comment = ticket_comment_repository.create_comment(
            db=db, obj_in=comment_in, user_id=current_user.id, ticket_id=ticket_id
        )

        # Notify assigned user if different from commenter
        if ticket.asignado_a_id and ticket.asignado_a_id != current_user.id:
            assigned_user = (
                db.query(User).filter(User.id == ticket.asignado_a_id).first()
            )
            if assigned_user:
        ...
        if (
            ticket.reportado_por_id
            and ticket.reportado_por_id != current_user.id
            and ticket.reportado_por_id != ticket.asignado_a_id
        ):
            reporter_user = (
                db.query(User).filter(User.id == ticket.reportado_por_id).first()
            )
            if reporter_user:
        ...
        raise HTTPException(
            ...
            detail=f"Error al crear el comentario: {e}",
        )
...
@router.get(
    ...
    description="Retrieves all comments associated with a specific ticket, ordered by creation date.",
)
def get_ticket_comments(
    ...
    comments = ticket_comment_repository.get_comments_by_ticket_id(
...
@router.post(
    ...
    summary="Receive FortiSIEM incident data",
    description="Endpoint to receive incident data from FortiSIEM via webhook. This endpoint processes the incoming XML data and can be extended to automatically create or update tickets based on the incident details.",
)
async def receive_fortisiem_incident(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error processing FortiSIEM incident: {e}"
        )
...
@router.put(
    "/{ticket_id}/remediate",
    response_model=TicketInDB,
    summary="Assign and set ticket to 'In Progress'",
    description="Assigns the specified ticket to the current authenticated user and sets its status to 'En Progreso' (In Progress), indicating the user is taking action to remediate the issue.",
)
async def remediate_ticket(
    ...
    ticket_in = TicketUpdate(asignado_a_id=current_user.id, estado="En Progreso")
...
@router.delete(
    ...
    description="Deletes a specific ticket identified by its ID. This action is typically restricted to users with appropriate permissions (e.g., 'Admin').",
)
def delete_ticket(
    ticket_id: int,
    db: Session = Depends(deps.get_db),
    ...
    ticket = ticket_service.delete_ticket(
        db=db, ticket_id=ticket_id, current_user_id=current_user.id
    )
    if not ticket:
        raise HTTPException(
            status_code=404,
