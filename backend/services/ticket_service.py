...
from api.routers.fortisiem import parse_raw_log_content # Importar la función de parseo
...
class TicketService:
    def get_paginated_tickets(
        ...
        for ticket_obj, first_name, last_name, raw_log in tickets_with_details:
            ...
            if first_name and last_name:
                ...
                ticket_in_db.reportado_por_nombre = "Sistema" if ticket_obj.reportado_por_id is None else "Desconocido"
    ...
    def get_ticket(self, db: Session, ticket_id: int, current_user_id: int) -> Optional[TicketInDB]:
        """
        ...
        ticket_with_reporter = ticket_repository.get_ticket_with_details(db, ticket_id=ticket_id)
        ...
        if ticket_obj.estado == "Nuevo" and ticket_obj.asignado_a_id == current_user_id:
            ...
            audit_log_data = AuditLogBase(
                ...
                detalle=json.dumps({"cambios": {"estado": {"old": old_estado, "new": "Abierto"}}, "reason": "Visto por asignado"})
            )
        ...
        evidence_records = ticket_repository.get_evidence_for_ticket(db, ticket_id=ticket_obj.id)
        ticket_in_db.evidencia = [
            {"id": ev.id, "nombre_archivo": ev.nombre_archivo, "ruta_almacenamiento": ev.ruta_almacenamiento}
    ...
    async def create_ticket(self, db: Session, ticket_data: TicketCreate, files: List[UploadFile], current_user_id: int) -> TicketInDB:
        # Create the ticket record in the database
        # This also generates a unique ticket_uid (e.g., TCK-YYYY-NNNNNN)
        created_ticket = ticket_repository.create_with_owner(db=db, obj_in=ticket_data, current_user_id=current_user_id)
        ...
        audit_log_data = AuditLogBase(
            ...
            detalle=json.dumps({"resumen": created_ticket.resumen, "estado": created_ticket.estado})
        )
        audit_log_repository.create(db, obj_in=audit_log_data)

        # Retrieve the full ticket details including reporter name, evidence, and raw logs
        full_ticket = self.get_ticket(db, ticket_id=created_ticket.id, current_user_id=current_user_id)
    ...
    def _check_ticket_update_permissions(self, current_user: User, db_ticket: Ticket, ticket_in: TicketUpdate):
        """
        ...
        if not is_admin_or_lider:
            ...
            if 'asignado_a_id' in update_data and update_data['asignado_a_id'] != db_ticket.asignado_a_id:
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
    ...
    def delete_ticket(self, db: Session, ticket_id: int, current_user_id: int) -> Optional[TicketInDB]:
        ...
        ticket_to_delete = self.get_ticket(db, ticket_id=ticket_id, current_user_id=current_user_id)
        ...
        deleted_ticket_db = ticket_repository.remove(db, id=ticket_id)
