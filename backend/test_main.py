...
def test_create_ticket_authenticated(test_client_with_admin):
    ...
    read_response = client.get(
        f"/api/v1/tickets/{created_ticket['id']}", headers=headers
    )
...
def test_read_tickets_search(test_client_with_admin):
    ...
    search_response = client.get(
        "/api/v1/tickets?search=Unique Search Term", headers=headers
    )
    ...
    search_response_desc = client.get(
        "/api/v1/tickets?search=search testing", headers=headers
    )
    ...
    no_results_response = client.get(
        "/api/v1/tickets?search=nonexistentticket123", headers=headers
    )
