...
class UserUpdate(UserBase):
    ...
    avatar_url: Optional[str] = Field(None, max_length=255)
    password: Optional[str] = Field(None, min_length=8, max_length=100) # For password change
...
class UserPublic(UserInDBBase):
    ...
    def generate_avatar_url(cls, values):
        if not values.get("avatar_url"):
            ...
            values["avatar_url"] = f"https://ui-avatars.com/api/?name={quote_plus(name)}&background=random"
        return values
